...
namespace GameServer.PacketManager
{
    public class PM_Saves : PM_Base
    {
        ...
        public static bool CheckIfUserHasSave(ServerClient client)
        {
            ...
            {
                if (Path.GetFileNameWithoutExtension(save) == client.GetData<PlayerFile>().Username) return true;
            }
        ...
        public static void ResetClientSave(ServerClient client)
        {
            ...
            {
                ResponseShortcutManager.SendIllegalPacket(client, $"Player {client.GetData<PlayerFile>().Username}'s save was attempted to be reset while the player doesn't have a save");
                return;
            }

            client.Listener.MarkForDisconnect();
            ResetPlayerData(client, client.GetData<PlayerFile>().Username);
        }
        ...
        {
            string savePath = Path.Combine(Master.SavesPath, client.GetData<PlayerFile>().Username + CommonValues.DefaultSaveFormat);
