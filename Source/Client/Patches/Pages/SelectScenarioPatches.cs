    ...
    public static bool DoPre(Rect rect, Page_SelectScenario __instance)
    {
        ...
        if (!SessionHandler.IsGeneratingFreshWorld && SessionHandler.CurrentScenario.IsEnforced)
        {
            ...
            else
            {
                ...
                RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("MESSAGE", ["Scenario will be forced by the server"], toDo));
...
public static class Patch_Page_SelectScenario_GoToScenarioEditor
{
    ...
    public static bool DoPre()
    {
        ...
        {
            RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("ERROR", ["This server doesn't allow custom scenarios!"]));
            return false;
        }
    }
}
