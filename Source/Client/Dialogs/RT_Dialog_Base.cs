...
{
    private static Window CurrentDialog { get; set; } = null;

    public static Window PreviousDialog { get; private set; } = null;

    protected static Vector2 DefaultButtonSize { get; private set; } = new(250f, 38f);

    public static Vector2 SmallButtonSize { get; private set; } = new(150f, 38f);

    protected static Vector2 SmallerButtonSize { get; private set; } = new(137f, 38f);

    protected static Vector2 TinyButtonSize { get; private set; } = new(47f, 25f);

    protected static Vector2 SlimButtonSize { get; private set; } = new(100f, 38f);

    protected static Vector2 LongButtonSize { get; private set; } = new Vector2(100f, 25f);

    protected string Title { get; set; } = string.Empty;

    protected string Description { get; set; } = string.Empty;

    protected Action OnAccept { get; set; } = null;

    protected Action OnCancel { get; set; } = null;

    protected bool AcceptsInput => StartAcceptingInputAtFrame <= Time.frameCount;

    public int StartAcceptingInputAtFrame { get; set; }

    protected Vector2 ScrollPosition = Vector2.zero;

    protected RT_Dialog_Base() 
    { 
        forcePause = true;
