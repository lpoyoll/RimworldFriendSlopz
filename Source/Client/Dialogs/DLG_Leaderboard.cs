...
namespace GameClient.Dialogs
{
    public class DLG_Leaderboard : DLG_Base
    {
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            if (Widgets.ButtonText(DLG_Base.GetRectForLocation(rect, SlimButtonSize, RectLocation.BottomCenter, true), "Close"))
            {
        ...
        private void FillMainRect(Rect mainRect)
        {
            ...
            for (int i = 0; i < Elements.Count(); i++)
            {
                if (num > num2 && num < num3)
                {
                    ...
                    DrawCustomRow(rect, Elements.ElementAt(i), num4);
                }
        ...
        private void DrawCustomRow(Rect rect, KeyValuePair<string, double> element, int index)
        {
            Text.Font = GameFont.Small;
            Rect fixedRect = new Rect(new Vector2(rect.x, rect.y + 5f), new Vector2(rect.width - 16f, rect.height - 5f));
            if (index % 2 == 0) Widgets.DrawHighlight(fixedRect);

            Widgets.Label(fixedRect, $"{element.Key} - {element.Value} points");
        }
    }
}
