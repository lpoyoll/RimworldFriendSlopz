...
public class ModConfigSetter : Mod
{
    ...
    public override void DoSettingsWindowContents(Rect inRect)
    {
        ...
        listingStandard.CheckboxLabeled("[When Playing] Mute incoming chat messages", ref ModConfigGetter.MuteChatSoundBool, "Mute chat messages");
    ...
    private void ShowVerboseFloatMenu()
    {
        ...
        List<Tuple<string, CommonEnumerators.VerboseMode>> verboseModes =
        [
    ...
    {
        List<FloatMenuOption> list = [];
        List<Tuple<string, CommonEnumerators.EnforcedSimulatedLag>> verboseModes =
        [
    ...
    private void ShowExportAccountQuestion()
    {
        Action toDo = delegate
        {
            ...
            catch { RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("MESSAGE", ["Path couldn't be found!"])); }
        };

        RT_Dialog_Base.PushNewDialog(new RT_Dialog_Inputs("Choose where to export the file at", ["Path"], [false], toDo));
    }
    ...
    private void ShowResetAccountQuestion()
    {
        RT_Dialog_YesNo dialog = new RT_Dialog_YesNo("Are you sure you want to RESET your ACCOUNT?",
            delegate
            {
                ...
                RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("MESSAGE", ["Account has been reset"]));
            });
