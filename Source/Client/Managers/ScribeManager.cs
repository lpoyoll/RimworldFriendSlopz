...
public static class ScribeManager
{
    ...
    private const string ScribeTreeName = "T";

    private const string ScribeNodeName = "N";
    ...
    public static HumanFile HumanToString(Pawn human)
    {
        ...
        humanFile.ScribeData = SerializeToString(human, SerializableType.Thing);
    ...
    {
        Pawn pawn = (Pawn)SerializeFromString<Pawn>(file.ScribeData);

        if (ModsConfig.IdeologyActive)
        {
            Ideo ideo = (Ideo)SerializeFromString<Ideo>(file.IdeologyData);
