...
public static class GuildManager
{
    ...
    public static void OnFactionOpen()
    {
        ...
        RT_Dialog_Buttons d1 = new RT_Dialog_Buttons("Guild Management", "Manage your guild from here",
            ...
            [delegate { r3(); }, delegate { RT_Dialog_Base.PushNewDialog(d2); }, delegate { RT_Dialog_Base.PushNewDialog(d3); }
            ],
            null);
    ...
    public static void OnNoFactionOpen()
    {
        Action r2 = delegate
        {
            ...
            {
                RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("ERROR", ["Guild name is invalid! Please try again!"
                ]));
            }
        ...
        RT_Dialog_Inputs d2 = new RT_Dialog_Inputs("New Guild Name", ["Input the name of your new guild"], [false], r2);
    ...
    public static void OnFactionOpenOnMember()
    {
        ...
        RT_Dialog_Buttons d2 = new RT_Dialog_Buttons("Power Management Menu", "Choose what you want to manage",
            ["Promote", "Demote"],
            [delegate { RT_Dialog_Base.PushNewDialog(d4); }, delegate { RT_Dialog_Base.PushNewDialog(d5); }],
        ...
        RT_Dialog_Buttons d1 = new RT_Dialog_Buttons("Management Menu", "Choose what you want to manage",
            ["Powers", "Kick"],
            [delegate { RT_Dialog_Base.PushNewDialog(d2); }, delegate { RT_Dialog_Base.PushNewDialog(d3); }]);
    ...
    private static void OnDeleteFaction()
    {
        ...
        RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("MESSAGE", ["Your guild has been deleted!"]));
    }

    private static void OnFactionNameInUse()
    {
        RT_Dialog_Wait.Instance.Close();
        RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("ERROR", ["That guild name is already in use!"]));
    }
    ...
    private static void OnFactionGetKicked()
    {
        ...
        RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("MESSAGE", ["You have been kicked from your guild!"]));
    }

    private static void OnFactionAdminProtection()
    {
        RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("ERROR", ["You can't do this action as a guild admin!"]));
    }

    private static void OnFactionPromote()
    {
        RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("MESSAGE", ["You have been promoted in your guild!"]));
    }

    private static void OnFactionDemote()
    {
        RT_Dialog_Base.PushNewDialog(new RT_Dialog_Message("MESSAGE", ["You have been demoted in your guild!"]));
    }
    ...
    private static void OnFactionMemberList(PlayerGuildData factionManifest)
    {
        ...
        for (int i = 0; i < factionManifest._guild.GuildMembers.Count; i++)
        {
            ...
            toDisplay.Add($"{member.Username} - {member.Rank}");
        }
