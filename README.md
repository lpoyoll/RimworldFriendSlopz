...
<p align="center">
  ...
  <img alt="Release" src="https://img.shields.io/badge/release-v0.1.28-d97738" />
...
v0.1.28 is the matched client/server release for live shared-tile play.

- Creates the server log directories before printing the build banner, fixing the v0.1.27 first-start crash when `Logs/System` did not exist.
...
- Requires the exact matching v0.1.28 / `RJ26` server before private pawn or world-state replication begins.
...
[Download Rimjob v0.1.28](https://github.com/lpoyoll/RimworldFriendSlopz/releases/tag/v0.1.28)

## Installation

Rimjob requires **Harmony**.

For Windows, use the [v0.1.28 MSI](https://github.com/lpoyoll/RimworldFriendSlopz/releases/download/v0.1.28/Rimjob-v0.1.28-x64.msi). It installs the client under the selected RimWorld installation at `Mods/Rimjob` and the exact matched server in the Desktop `Rimjob Server` folder.

Both clients and the server must use v0.1.28. From this release onward, `Update.exe` also updates the installed Desktop server when it is present and closed.
